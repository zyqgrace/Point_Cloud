...
def sample_and_group(npoint, radius, nsample, xyz, points):
    """
    :param npoint: int, number of points sampled
    :param radius: float, radius of the ball query
    :param nsample: int, maximum number of points in the ball query
    :param xyz: torch.Tensor, (B, N, 3)
    :param points: torch.Tensor, (B, N, C)
    :return: new_xyz: (B, npoint, 3)
             new_points: (B, npoint, nsample, 3+C)
    """
    # Make sure xyz is contiguous
    xyz = xyz.contiguous()

    fps_idx = farthest_point_sample(xyz, npoint) # [B, npoint, C]
    new_xyz = gather_operation(xyz.transpose(1, 2).contiguous(), fps_idx)  # [B, D, npoint]
    new_xyz = new_xyz.transpose(1, 2).contiguous()

    if points is not None:
        points = points.contiguous()
        new_points = gather_operation(points.transpose(1, 2).contiguous(), fps_idx)
